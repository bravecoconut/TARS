...
async def change_name(session_id, new_name):
    try:
        ...
        return r_h(
            False,
            "Error occured when changing session name",
            str(e),
        )
...
async def toggle_pin(session_id):
    try:
        ...
        return r_h(
            False,
            "Error occured when toggleing pin",
            str(e),
        )
...
async def update_last_user_message(session_id, new_message):
    try:
        ...
        return r_h(True, "last message changed", _result.messages[user_index])
    except Exception as e:
        return r_h(
            False,
            "Error occured when changing last message",
            str(e),
        )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        result = await update_last_user_message(
            session_id="6a8d47b3e521351855556c50",
            new_message={"e": 43},
        )
        print(result)
